...
def main() -> None:
    ...
    # The taxonomy is regenerated by an LLM in Phase 4, so its size and ids are
    # not fixed facts — these check the API against the artifacts it was built
    # from, which stays true whichever model produced them.
    taxonomy = pd.read_csv(REPO_ROOT / "data" / "preference_classification.csv")
    mapping = pd.read_csv(REPO_ROOT / "data" / "topic_preference_mapping.csv")
    mapped_ids = set(mapping["preference_id"].astype(str))
    unmapped_ids = set(taxonomy["preference_id"].astype(str)) - mapped_ids

    code, prefs = get("/api/preferences")
    check(
        "GET /api/preferences returns every preference in the taxonomy",
        code == 200 and len(prefs) == len(taxonomy),
        f"{len(prefs)} vs {len(taxonomy)} in preference_classification.csv",
    )
    check(
        "preferences with no mapped topic still appear, with zero evidence",
        all(
            p["review_count"] == 0
            for p in prefs
            if p["preference_id"] in unmapped_ids
        ),
        str([(p["preference_id"], p["review_count"]) for p in prefs]),
    )
    code, mapped_only = get("/api/preferences", include_unmapped=False)
    check(
        "include_unmapped=false drops the unmapped ones",
        len(mapped_only) == len(mapped_ids),
        f"{len(mapped_only)} vs {len(mapped_ids)} mapped",
    )

    # Pick a preference that actually carries topics, rather than assuming one.
    topics_per_pref = mapping["preference_id"].astype(str).value_counts()
    busiest = str(topics_per_pref.index[0])
    code, pref = get(f"/api/preferences/{busiest.lower()}")
    check(
        "preference lookup is case-insensitive and joins topics",
        code == 200
        and pref["preference"]["preference_id"] == busiest
        and len(pref["topics"]) == int(topics_per_pref.iloc[0]),
        f"{code} {busiest}: {len(pref.get('topics', [])) if code == 200 else '-'} topics",
    )
    check(
    ...
    check(
        "taxonomy tree holds every preference",
        leaf_count == len(taxonomy),
        f"{leaf_count} leaves vs {len(taxonomy)} preferences",
    )
    ...
    coverage_csv = pd.read_csv(REPO_ROOT / "data" / "phase5_direction2_coverage_checks.csv")
    expected_under = int((coverage_csv["coverage_status"] == "under_coverage").sum())
    check(
        "validation reports the same under-coverage count as Phase 5",
        val["direction2_coverage"]["under_coverage"] == expected_under,
        f"{val['direction2_coverage'].get('under_coverage')} vs {expected_under} in the CSV",
    )
    check(
    ...
    check(
        "sensitivity recommends a minimum corpus size from its own ladder",
        code == 200 and sens["recommended_min_corpus_size"] in ladder,
        f"{sens.get('recommended_min_corpus_size')} not in {sorted(ladder)}",
    )
    code, sensmap = get("/api/research/sensitivity/mappings", sample_size=max(ladder))
